...
function dragEnded(origin, width, node, d) {
  ...
  if (!legal) {
    ...
    node.attr("cx", (d.x = cellCoordinates.x + width / 2)).attr("cy", (d.y = cellCoordinates.y + width / 2));
  } else {
...
function getJumpedPiece(cells, pieces, from, to) {
  ...
  if (Math.abs(distance.x) == 2) {
    var jumpRow = from.row + Math.sign(distance.y);
    var jumpCol = from.col + Math.sign(distance.x);
...
function isMoveLegal(cells, pieces, piece, from, to) {
  ...
  if (Math.abs(distance.x) == 2) {
    ...
    var pieceState = Math.floor(piece.state);
    var jumpedState = Math.floor(jumpedPiece.state);
    if (pieceState != -jumpedState) {
      return false;
    }
  }
  if (Math.floor(piece.state) === piece.state && Math.sign(piece.state) != Math.sign(distance.y)) {
    return false;
  }
  return true;
}
...
function drawBoard(origin, cellWidth, boardCanvas) {
  ...
  var drag = d3.drag().on("start", dragStarted).on("drag", dragged).on("end", dragEndedDimensions);
  boardCanvas
    .append("g")
    .selectAll("circle")
    .data(pieces)
  ...
  d3.select("body").append("div").attr("id", "divScoreboard").style("font-size", "36").html("SCOREBOARD");
  d3.select("#divScoreboard").append("div").style("font-size", "24").attr("id", "winner");
  d3.select("#divScoreboard").append("div").attr("id", "redScore").style("font-size", "18").html("Red: 12");
  d3.select("#divScoreboard").append("div").attr("id", "blackScore").style("font-size", "18").html("Black: 12");
  return boardState;
}
...
function get_player_pieces(player, target_board) {
  ...
  for (var i = 0; i < target_board.pieces.length; i++) {
    ...
    if (piece.state === player || piece.state === player + 0.1 || piece.state === player - 0.1) {
...
function get_available_piece_moves(target_board, target_piece, player) {
    ...
    // Verifica movimentos de deslizamento (movimentos simples) para frente
    var x = [-1, 1];
    ...
    // Verifica movimentos de salto (movimentos mais longos)
    x = [-2, 2];
    x.forEach(function(entry) {
    ...
    // Verifica movimentos especiais de reis (peças que alcançaram o outro lado do tabuleiro)
    if (Math.abs(from.state) === 1.1) {
        for (var dx = -7; dx <= 7; dx++) {
            for (var dy = -7; dy <= 7; dy++) {
                if (Math.abs(dx) === Math.abs(dy) && dx !== 0) {
                    var cell_index = get_cell_index(target_board, from.col + dx, from.row + dy);
                    if (cell_index >= 0) {
                        var to = target_board.cells[cell_index];
                        if (isMoveLegal(target_board.cells, target_board.pieces, from, from, to)) {
                            move = {move_type: 'slide', piece: player, from: {col: from.col, row: from.row}, to: {col: to.col, row: to.row}};
        ...
        // Verifica movimentos de salto para os reis
        for (var dx = -7; dx <= 7; dx++) {
            for (var dy = -7; dy <= 7; dy++) {
                if (Math.abs(dx) === Math.abs(dy) && dx !== 0 && Math.abs(dx) > 1) {
                    var cell_index = get_cell_index(target_board, from.col + dx, from.row + dy);
                    if (cell_index >= 0) {
                        var to = target_board.cells[cell_index];
                        if (isMoveLegal(target_board.cells, target_board.pieces, from, from, to)) {
